...
const AdBanner = ({ section, data = [] }) => {
    const { canEdit, onRemove } = useSectionEdit();

    // Si no hay anuncios en la sección, no mostramos nada (o un placeholder en modo edición)
    if (!data.length) {
        if (canEdit) {
            return (
                <div className="widget ad-widget-placeholder">
                    <p>Espacio para Anuncio ({section.section_type}).<br />Añada un anuncio desde el panel de acciones de la sección.</p>
                </div>
            );
        }
        return null;
    ...
    // Suponemos que cada sección de anuncio muestra un solo item a la vez.
    const ad = data[0];

    // Para no repetir código, definimos el contenido del anuncio aquí.
    // Este JSX será el mismo para todos los formatos.
    const adContent = (
        <>
            ...
            {canEdit && (
                <button
                    className="remove-item-from-section-btn"
                    title="Quitar este anuncio de la sección (no lo elimina permanentemente)"
    ...
    // El 'switch' es el cerebro que decide qué contenedor usar.
    switch (section.section_type) {
        case 'ad-banner':
            // Formato para un banner ancho, usa un tag <section>
            return (
                <section className="ad-container ad-banner-container">
                    {adContent}
                </section>
            );

        case 'ad-skyscraper':
            // Formato para un banner vertical, usa un div con clase 'widget'
            return (
                <div className="widget ad-skyscraper-container">
                    {adContent}
                </div>
            );

        case 'ad-large':
            // Formato para un anuncio grande, tipo widget
            return (
                <div className="widget ad-large-container">
                    {adContent}
                </div>
            );

        case 'ad-small':
            // Formato para un anuncio pequeño, tipo widget
            return (
                <div className="widget ad-small-container">
                    {adContent}
                </div>
            );

        default:
            // Si el tipo no coincide con ninguno, no renderizamos nada.
            return null;
    }
};
