    ...
    return canEdit ? (
      <div className="widget audio-news">
        <div className="widget-header">
          {/* 2. Usamos el título dinámico de la sección */}
...
/*// src/components/layout/public/home/AudioNewsWidget.jsx
import React, { useState, useRef, useEffect } from "react";
import { useSectionEdit } from "../../../../context/SectionEditContext";

const ITEMS_PER_PAGE = 4;

const AudioNewsWidget = ({ data: audios = [] }) => {
  // Contexto de edición (añadir, eliminar, editar)
  const { canEdit, onAddItem, onRemove, onEdit } = useSectionEdit();
  ...
  // Reiniciar paginación al cambiar datos
  useEffect(() => {
    setCurrentPage(1);
  }, [audios]);

  // Si no hay audios, mostrar estado vacío solo si se puede editar
  if (!audios || audios.length === 0) {
    return canEdit ? (
      <div className="widget audio-news">
        <div className="widget-header">
        ...
        {pageItems.map((audio) => (
          <div key={audio.audio_code} className="audio-news-item">
            <div className="compact-row">
              ...
              <h5 className="audio-title">{audio.audio_title}</h5>
            </div>
            ...
            {canEdit && (
              <div className="item-actions">
                <button
                  className="edit-item-btn"
                  title="Editar nota de audio"
                  onClick={() => onEdit(audio, 'audio')}
                >
                  <i className="fas fa-pen" />
                </button>
                <button
                  className="delete-item-btn"
                  title="Eliminar nota de audio"
                  onClick={() => onRemove(audio.audio_code, 'audio')}
                >
                  <i className="fas fa-trash" />
                </button>
              </div>
